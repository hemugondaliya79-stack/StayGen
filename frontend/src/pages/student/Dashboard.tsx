    ...
    <div className="space-y-6">
      ...
      <div className="gradient-bg rounded-2xl p-6 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="text-white">
            ...
            {student?.roomId && <p className="text-white/80 text-sm mt-1">📍 Room {student.roomId.roomNumber}, Floor {student.roomId.floor}</p>}
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { icon: CreditCard, label: 'Due Fees', value: data?.pendingFees?.length ? formatCurrency(data.pendingFees.reduce((s: number, f: any) => s + f.amount, 0)) : '₹0', color: 'bg-amber-500', link: '/student/fees' },
          ...
          <Link to={stat.link} key={i}>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-100 dark:border-slate-700 hover:shadow-lg hover:shadow-indigo-500/5 transition-all cursor-pointer">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/student/mess" className="text-xs text-indigo-600 hover:text-indigo-700 font-medium">View Week →</Link>
          </div>
          {todayMenu ? (
            <div className="space-y-2">
              {[['☀️ Breakfast', todayMenu.breakfast], ['🍱 Lunch', todayMenu.lunch], ['🍪 Snacks', todayMenu.snacks], ['🌙 Dinner', todayMenu.dinner]].map(([label, meal], i) => (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/student/fees" className="text-xs text-indigo-600 hover:text-indigo-700 font-medium">View All →</Link>
          </div>
          {isLoading ? (
            ...
            <div className="text-center py-6">
              <p className="text-emerald-500 text-2xl mb-2">✓</p>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/student/notices" className="text-xs text-indigo-600 hover:text-indigo-700 font-medium">View All →</Link>
          </div>
          {notices?.length ? (
            <div className="space-y-3">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/student/attendance" className="text-xs text-indigo-600 hover:text-indigo-700 font-medium">View Details →</Link>
        </div>
        ...
        <div className="flex gap-6 text-sm text-slate-500">
          ...
          {(attendance?.percentage || 0) < 75 && (
            <span className="text-red-500 font-medium">⚠️ Below 75% threshold</span>
          )}
        </div>
      </div>
    </div>
